...
<body>
    ...
    <main class="flex col">
        ...
        <div class="block mr-t15 flex col gap10 col-center">
            <h2>Изменение фамилии</h2>
            <div class="flex row gap10">
                <p>Текущая фамилия:</p>
                <p>
                    <?php
                    $current = $mysqli->query('SELECT SName FROM users WHERE id='.$user);
                    $current = $current->fetch_object();
                    echo $current->SName;
                    ?>
                </p>
            </div>
            <form action="../handlers_admin/changeSName.php" method="post" class="flex col col-center gap15" style="width:70%;">
                <input name="id_user" type="text" value="<?=$user?>" hidden>
                <input name="new" type="text" class="inp width100p" placeholder="Новая фамилия" require>
                <div class="error">
                    <?php
