    ...
    <main class="flex col">
        ...
        <div class="block mr-t15 flex col gap10 col-center">
            ...
            <div class="flex row gap10">
                ...
                <p>
                    <?php
                    $current = $mysqli->query('SELECT TName FROM users WHERE id='.$user);
                    $current = $current->fetch_object();
                    if(empty($current->TName)){
                        $current->TName = 'Отсутствует';
                    }
                    echo $current->TName;
                    ?>
                </p>
            </div>
            <form action="../handlers_admin/changeTName.php" method="post" class="flex col col-center gap15" style="width:70%;">
                <input name="id_user" type="text" value="<?=$user?>" hidden>
                <input name="new" type="text" class="inp width100p" placeholder="Новое отчество, оставить пустым если такогого нет" require>
                <div class="error">
                    <?php
